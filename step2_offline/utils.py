...
def tokenize(query):
    temp = tokenization.convert_to_unicode(query).strip()
...
def entity_label_tokenize(entity_label):
    feat_str = tokenization.convert_to_unicode(entity_label).strip()
    dict_feat_list = []
    if feat_str == '_':
        dict_feat_list.append('_')
    else:
        dict_feat_list = feat_str.split('_')
